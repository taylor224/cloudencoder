...
func setVideoFlags(opt videoOptions, videoStreamData stream, disableHWAccel bool) []string {
	...
	if opt.FrameRate == "auto" {
		...
		one, _ := strconv.Atoi(splitted[0])
		two, _ := strconv.Atoi(splitted[1])
		RFrameRate := one / two
		
		args = append(args, []string{"-r", strconv.Itoa(RFrameRate)}...)
...
func setVideoFilters(vopt videoOptions, opt filterOptions, videoStreamData stream, disableHWAccel bool) string {
	...
	if vopt.Size != "" && vopt.Size != "source" {
		...
		if vopt.Size == "custom" {
			...
			strScaledWidth, _ := strconv.ParseFloat(scaledWidth, 64)
			strScaledHeight, _ := strconv.ParseFloat(scaledHeight, 64)
